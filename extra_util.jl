...
function concatenate_MPOs(mpo1::MPO, site1::Vector{<:Index}, mpo2::MPO, site2::Vector{<:Index})
    N1 = length(mpo1)
    N2 = length(mpo2)
    ...
    for i in 1:N2
        ...
        if i == 1
            ...
            if !isnothing(l2)
                ...
                tensors[N1 + 1] = T * setelt(mid_link => 1)
            end
        else
            tensors[N1 + i] = T
        end
    ...
    return MPO(tensors), new_sites
end

function build_shift_mpo(sites, q,p)
    N = length(sites)
    ...
    if p == 1 
       mpo[1] *=  onehot(links[1] => 1) 
    end
    if p == 2 
       mpo[1] *=  (onehot(links[1] => 1) + onehot(links[1] => 2) )
    end
    return mpo
end
################################################################################
